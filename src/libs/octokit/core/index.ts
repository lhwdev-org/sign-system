...
import {
  Collection,
  HookCollection,
} from "https://cdn.skypack.dev/before-after-hook?dts";
import { request } from "../request/index.ts";
import { graphql, withCustomRequest } from "../graphql/index.ts";
import { createTokenAuth } from "../auth-token/index.ts";
...
export class Octokit {
  ...
  constructor(options: OctokitOptions = {}) {
    ...
    requestDefaults.headers["user-agent"] = [
      options.userAgent,
      `octokit-core.js/${VERSION} Deno/${Deno.version.deno}`,
    ]
      .filter(Boolean)
      .join(" ");
