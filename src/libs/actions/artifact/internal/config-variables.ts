...
export function getRuntimeToken(): string {
  const token = Deno.env.get("ACTIONS_RUNTIME_TOKEN");
  if (!token) {
...
export function getRuntimeUrl(): string {
  const runtimeUrl = Deno.env.get("ACTIONS_RUNTIME_URL");
  if (!runtimeUrl) {
...
export function getWorkSpaceDirectory(): string {
  const workspaceDirectory = Deno.env.get("GITHUB_WORKSPACE");
...
export function getRetentionDays(): string | undefined {
  return Deno.env.get("GITHUB_RETENTION_DAYS");
}
