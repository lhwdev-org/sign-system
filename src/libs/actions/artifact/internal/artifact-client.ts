...
import {
  ...
  UploadSpecification,
} from "./upload-specification.ts";
import { UploadHttpClient } from "./upload-http-client.ts";
import { UploadResponse } from "./upload-response.ts";
import { UploadOptions } from "./upload-options.ts";
import { DownloadOptions } from "./download-options.ts";
import { DownloadResponse } from "./download-response.ts";
import {
  createDirectoriesForArtifact,
  createEmptyFilesForArtifact,
} from "./utils.ts";
import { checkArtifactName } from "./path-and-artifact-name-validation.ts";
import { DownloadHttpClient } from "./download-http-client.ts";
import { getDownloadSpecification } from "./download-specification.ts";
import { getWorkSpaceDirectory } from "./config-variables.ts";
...
export class DefaultArtifactClient implements ArtifactClient {
  ...
  async uploadArtifact(
    ...
    return await core.group(`Upload artifact ${name}`, async () => {
      core.info(
...
For more detailed logs during the artifact upload process, enable step-debugging: https://docs.github.com/actions/monitoring-and-troubleshooting-workflows/enabling-debug-logging#enabling-step-debug-logging`,
      ...
      if (uploadSpecification.length === 0) {
        ...
        const uploadResult = await uploadHttpClient
...
Note: The size of downloaded zips can differ significantly from the reported size. For more information see: https://github.com/actions/upload-artifact#zipped-artifact-downloads \r\n`,
  ...
  async downloadArtifact(
    ...
    return await core.group(`Download artifact ${name}`, async () => {
  ...
  async downloadAllArtifacts(
    ...
    return await core.group(`Download all artifacts`, async () => {
      const downloadHttpClient = new DownloadHttpClient();
